...
def display_status(data, use_color):
    ...
    if not isinstance(indicator_key, str):
        indicator_key = "none"
    ...
    for component in visible_components:
        ...
        if not isinstance(comp_status, str):
...
def _get_visible_components(data):
    ...
    for c in raw:
        ...
        if not isinstance(name, str):
            c = dict(c, name="Unknown")
        # Normalize non-string status so both display and exit-code logic agree
        comp_status = c.get("status", "operational")
        if not isinstance(comp_status, str):
            c = dict(c, status="operational")
        valid.append(c)
...
def main():
    ...
    # Exit 2 if any component is degraded OR page-level status indicates an issue
    ...
    components_degraded = not all_operational(visible)
    page_degraded = indicator not in ("none", "")

    if components_degraded or page_degraded:
        sys.exit(2)
